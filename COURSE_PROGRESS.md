# Course Progress

Single source of truth for where I am in the course.
Updated at the end of every episode (step 10 of the [workflow](LEARNING_GUIDE.md#4-the-per-episode-workflow)).

**Status legend:** ✅ Complete · ✅* Complete with deferred follow-ups · 🔄 In progress · ⬜ Not started
**Confidence:** 1 = lost · 2 = shaky · 3 = could follow along · 4 = solid · 5 = could rebuild from scratch
**Assessment tier:** 0 = concept check only · 1 = + coding challenge · 2 = + debugging challenge

Confidence is rated *after* the coding challenge, never before — see [LEARNING_GUIDE.md §5](LEARNING_GUIDE.md#5-the-episode-assessment).

---

## At a glance

| | |
|---|---|
| **Episodes completed** | 2 |
| **Current episode** | Episode 03 — awaiting lecture notes |
| **Course length** | 8 weeks, ~40–45 episodes |
| **Started** | 2026-08-01 |

---

## Week 1 — Foundations & Setup

| Ep | Title | Status | Tier | Confidence | Notes | Assessment |
|---|---|---|---|---|---|---|
| 01 | Course Intro + Tech Stack Setup | ✅ | 0 | 4 † | [notes](Week1/episode-01-intro-setup/notes.md) | [assessment](Week1/episode-01-intro-setup/assessment.md) |
| 02 | Understand Your First LLM Call | ✅* | 2 | 4 | [notes](Week1/episode-02-first-llm-call/notes.md) | [assessment](Week1/episode-02-first-llm-call/assessment.md) |
| 03 | *(not yet provided)* | ⬜ | — | — | | |

† Self-rated 4, with a documented split: **4 mechanical / 3 conceptual**. The conceptual items are on the [revisit list](LEARNING_JOURNAL.md#revisit-list) and will be re-checked in the Week 1 review.

**Week 1 review:** ⬜ not started — run before Week 2 begins.

---

## Course roadmap

The eight themes below come from the Episode 01 overview. Episode-level detail is filled in as each lecture is provided — the course has not published a full episode list up front.

| Week | Theme | Status |
|---|---|---|
| 1 | Setup & LLM fundamentals | 🔄 |
| 2 | LLM fundamentals (continued) | ⬜ |
| 3 | RAG & document Q&A | ⬜ |
| 4 | AI agents & autonomous research agents | ⬜ |
| 5 | LangGraph (agent orchestration) | ⬜ |
| 6 | MCP (Model Context Protocol) servers | ⬜ |
| 7 | AI observability & evaluation | ⬜ |
| 8 | Deployment, fine-tuning & capstone project | ⬜ |

---

## Weak spots to revisit

Episodes rated **3 or below** land here automatically, so review targets are obvious at a glance.
See the [rolling revisit list](LEARNING_JOURNAL.md#revisit-list) for specific topics.

| Ep | Topic | Confidence | Revisited? |
|---|---|---|---|
| — | *nothing flagged yet* | | |

---

## Projects

Multi-episode applications live in [`projects/`](projects/README.md).

| Project | Status | Episodes drawn from |
|---|---|---|
| — | *none yet* | |
